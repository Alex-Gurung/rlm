...
## Prompt Presets and Store Modes
RLM exposes two knobs that are useful for benchmarking and comparisons:

- `prompt_preset`: `"default"` (current prompt set) or `"legacy"` (baseline prompts).
- `store_mode`: `"shared"` (default, shared store available) or `"none"` (store disabled).
...
rlm = RLM(
    ...
    store_mode="none",  # baseline behavior
)
```

Note: the shared store is scoped per completion; even with `persistent=True` the store resets each completion.
