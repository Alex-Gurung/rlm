...
def build_root_prompt(question: str, subagent_hint: bool, icl: bool) -> str:
    root_prompt = (
        ...
        "- Determine the question type first (e.g., spells, rolls, hits). Do not assume it's always about spell casts.\n"
        "- Extract occurrences directly from speaker lines that mention the target, then aggregate in Python.\n"
        "- When using sub-LLMs, first use small calls to understand structure (episode boundaries, speaker format), then send\n"
        "  structured chunks with a clear, specific sub-task tied to the question.\n"
        "- Keep sub-LLM inputs small (chunk episodes); do not send full episodes or the full context.\n"
        "- Include the exact question in sub-LLM prompts so the sub-task stays aligned with the goal.\n"
        "- If you request JSON via output_format='json', validate with parse_json() and retry on parse errors.\n"
        "- Do not ask for JSON if you only need 'yes'/'no' or plain text; keep output format consistent.\n"
        "- If you build sub-LLM prompts with Python string formatting and include JSON examples, escape braces ({{ }})\n"
        "  or avoid .format() to prevent formatting errors.\n"
